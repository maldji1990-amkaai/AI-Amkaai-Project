...
async function refreshGeneration(generationId: string) {
  const generation = await db.generation.findUnique({
    ...
    include: { videoJobs: true, project: true },
  });
  ...
  const completed = videoJobs.filter(
    (j) => j.status === "COMPLETED",
  ).length;

  const failed = videoJobs.some(
  ...
  if (failed) {
    await db.generation.update({
      ...
      data: {
        status: "FAILED",
        error: "One or more scene renders failed",
      },
    });
  ...
  if (total > 0 && completed === total) {
    ...
    try {
      ...
      console.warn(
        "Automatic post-production dispatch skipped:",
        error,
      );
    }
  }
...
export async function POST(req: Request) {
  try {
     ...
     * Serverless callback:
     *
     * video-dispatch stores the RunPod Serverless job ID
     * in externalJobId.
     *
     * The current handler may send the application job ID
     * as body.id. The URL already contains the trusted
     * application jobId, so accept either identifier.
     */
    if (
      job.externalJobId &&
      externalId &&
    ...
    const sceneId =
      typeof input.scene_id === "string"
        ...
        : job.generation?.metadata &&
          ...
          ? (job.generation.metadata as any).sceneId
          : null;
    ...
    if (
      ...
      await db.$transaction(async (tx) => {
        ...
        if (job.generationId) {
          await tx.generationStep.updateMany({
            where: {
